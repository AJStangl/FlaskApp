...
def auto_azure_thumbnail(image_id: str, width: int = 512, height: int = 512, smartCropping: bool = True):
...
def create_croppings_for_image():
	pass
...
if __name__ == '__main__':
	...
	for entity in tqdm(entities, total=len(entities)):
		...
		if f"{image_id}.jpg" in extant:
			print(f"Skipping {image_id}, already created...")
			entity['azure_thumbnail_path'] = f"data/image/azure/{image_id}.jpg"
			client.upsert_entity(entity)
			continue
		azure_thumbnail_path = auto_azure_thumbnail(image_id=entity['id'], smartCropping=False)
